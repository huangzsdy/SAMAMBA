    ...
    def setup_lr_schduler(self):
        ...
        if scheduler_type is None:
            ...
            if scheduler_type in ['MultiStepLR', 'MultiStepRestartLR']:
                ...
                scheduler_cfg = lr_opt['scheduler'].copy()
                max_lr = scheduler_cfg.pop('max_lr', 1e-3)
                pct_start = scheduler_cfg.pop('pct_start', 0.3)
                anneal_strategy = scheduler_cfg.pop('anneal_strategy', 'cos')
                div_factor = scheduler_cfg.pop('div_factor', 25.0)
                final_div_factor = scheduler_cfg.pop('final_div_factor', 1000.0)
                three_phase = scheduler_cfg.pop('three_phase', False)
