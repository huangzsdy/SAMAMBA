...
基于原始 train.py，添加渐进式解冻功能

使用方法:
    python train_gradual_unfreeze.py --config options/train.yaml
"""
...
def main():
    ...
    # 初始化渐进式解冻器
    gradual_unfreeze_opt = opt.get('model', {}).get('gradual_unfreeze', {})
    if gradual_unfreeze_opt.get('enabled', False):
        unfreezer = GradualUnfreezerSimple(
            model,
            unfreeze_epochs=gradual_unfreeze_opt.get('unfreeze_epochs', [150, 300]),
            lr_factors=gradual_unfreeze_opt.get('lr_factors', [0.1, 0.01])
        )
        logger.info("Gradual unfreezing enabled")
    else:
        unfreezer = None
